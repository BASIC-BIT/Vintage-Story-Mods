...
Symbol/effect follow-up: BASIC approved solid dot, hollow ring, diamond, and optional Hologram for every symbol. Existing symbol enum IDs are retained; new IDs append at 3-5. Effect is Plain by default, appended at protobuf 17 and stored in sceneEffect. Hologram uses cached translucent scan-line artwork and a six-second opacity shimmer bounded to 88-100 percent of that artwork. It uses the existing depth-tested renderer, retaining fog/range fade, size, height and target feedback. Text is unaffected. Preview shares the artwork; saved defaults and lock restrictions include effect. QA: compare all six symbols, verify ring center is hollow, toggle Plain/Hologram across colors, test walls/fades, and check save/reopen, pickup and next-marker defaults.
Review correction: cache opaque symbol interiors with baked brighter scan lines, then apply hologram transparency/shimmer through shader tint. Preview uses Cairo group opacity at a fixed shimmer phase. This avoids multiplying premultiplied interior colors twice. Full suite: 790 passed.
Final symbol/effect package SHA256 2954A7185D153E45BDDB2D67874FAC729B9C8D77567E0E96B2EE6F6460DF9A18 matches the passing test DLL; packaged JSON parses. Both review rechecks clear. Server/profile hashes verified during authorized QA refresh. Visual acceptance pending.
