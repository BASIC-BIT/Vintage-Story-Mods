...
3. **Pick up and move.** Unlock and break the marker, then place the dropped written item on a wall. Expect `removed`, `placed` with `reused` and `wall`, and one `moved`. The text should survive exactly as before. A locked or claim-denied break must produce no removal event.
...
5. **Turn analytics off.** Disable analytics through the existing consent control. Repeat placement/save/open/view. Expect no remote scene events; marker gameplay still works. Re-enable and check that new actions count without replaying observations from the disabled period. Disable Scene Markers in `/basic` and verify reporting stops along with normal feature behavior.
