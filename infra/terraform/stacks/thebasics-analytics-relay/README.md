...
Contract revision 6 adds Scene Marker action counts and `enable_scene_markers` config snapshots. Scene events remain installation-only even under personalized consent. The relay rejects pseudonyms and any fields outside the specific scene action's bounded property set. Reader opens and bubble impressions are client observations validated by the game server. They do not prove that a player read the text. See [Scene Marker metric definitions](../../../../docs/analytics/scene-markers.md).
