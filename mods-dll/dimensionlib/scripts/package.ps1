...
$projectFile = Join-Path $projectRoot "dimensionlib.csproj"
...
$dllFile = Join-Path $outputDir "dimensionlib.dll"
$pdbFile = Join-Path $outputDir "dimensionlib.pdb"
$modInfo = Get-Content $modInfoFile | ConvertFrom-Json
$version = $modInfo.version -replace '\.', '_' -replace '-', '_'
$zipFile = Join-Path $projectRoot "dimensionlib_$version.zip"
$logFile = Join-Path $solutionRoot "package.log"

$timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
"[$timestamp] DimensionLib package script started" | Out-File -FilePath $logFile
...
$msg = "Successfully created DimensionLib package at $zipFile"
