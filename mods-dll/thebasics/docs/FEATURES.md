...
- Creator and lock metadata survive world reload. A locked marker cannot be picked up until its creator or an admin unlocks it. Normal pickup and replacement preserve creator metadata; creative pick-block copies the content and creator metadata but clears the lock. Editing an unlocked marker does not transfer ownership. Locked markers resist explosions. Edits and unlocks are written to the server audit log.
