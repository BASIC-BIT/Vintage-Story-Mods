...
6. **Scene Markers** (P1)
   ...
   4. **Display Modes And Distance Fade** (P1)
      - Do: Compare When targeted, Always nearby, and On interaction. Set indicator distance to a short finite value, then Unlimited; set a separate nearby text distance. Walk in and out of both radii, aim at and away from each marker, and check occlusion behind a wall, including a marker placed flat against a wall viewed from a steep angle.
      - Expect: When targeted shows the bubble only while aimed at it. Always nearby shows it throughout its own text distance and fades independently of the indicator. On interaction shows no bubble at all. Indicators fade through the outer quarter of their distance, Unlimited stays visible within loaded terrain, and walls obscure the indicator. The bubble stays upright and screen-aligned above the indicator at every camera angle, is never cut by the wall behind it, and disappears entirely once a wall blocks the line of sight to the marker.
      - Watch for: the text distance field enabled in the wrong mode, a bubble in On interaction mode, indicators visible through terrain, a bubble diagonally clipped by or sunk into a wall instead of vanishing, a bubble drawn over an open dialog or the chat box, or flicker at the fade boundary.

   5. **Bubble Size, Title Icon, Show Description** (P1)
      - Do: With Show description in bubble off, then on, compare a short and a long multiline description at bubble size 40, 100, and 350 percent. Open the Icon picker, search a name, page through the catalog, choose an icon, then choose None. View from screen edges and steep camera angles.
   ...
   11. **Pickup And Re-Place Persistence** (P0)
       - Do: Unlock a written, appearance-customized, locked marker, then break and re-place it. In the primary editor change the symbol, color, sizes, distances, and display mode. Enter icon distance 100 and nearby text distance 12, then turn on both Unlimited switches while those values remain visible; click Set as my defaults, then Cancel and place a fresh crafted marker. Reopen and repeat the distance toggle sequence with different values, using Save & Close this time. Place another fresh marker. With Unlimited off, type 2048 or nonnumeric text into a distance field and try Cancel; confirm the discard prompt appears. Try Set as my defaults with an out-of-range size and from a secondary or locked editor. Restart the server and rejoin.
       - Expect: Set as my defaults confirms that future fresh markers use the chosen appearance and that the current marker still needs Save & Close. Cancel leaves the current marker unchanged, while the fresh marker inherits all explicitly chosen appearance values with empty text, including icon distance 100, nearby text distance 12, and both Unlimited settings. Save & Close applies its own visible distance values to only that marker and does not replace the remembered defaults. The action is absent from secondary/new editors and unavailable when locked; invalid values are refused. Title, body, display mode, every appearance setting, and creator survive pickup, re-place, and restart. Exactly one item drops, and the picked-up marker keeps its own appearance.
       - Watch for: defaults changing after an ordinary Save & Close, a false success confirmation after rejection, altered current-marker content or read state after setting defaults, duplicate or missing drops, reset appearance, transferred ownership, or a fresh marker inheriting someone else's content.
   ...
   13. **EnableSceneMarkers=false** (P0)
