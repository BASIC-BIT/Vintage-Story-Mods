...
public static class ConfigAdminSettingRegistry
{
    ...
    private static void AddCoreScalarSettings(List<ConfigAdminSettingDefinition> settings)
    {
        settings.AddRange(new[]
        {
            ...
            Bool("EnableDiceRollSounds", "Chat/RP", "Enable dice roll sounds", "Play a dice roll sound for eligible listeners.", ConfigAdminReloadBehavior.Live, c => c.EnableDiceRollSounds, (c, v) => c.EnableDiceRollSounds = v),
            ...
            Bool("EnableSceneMarkers", "Chat/RP", "Enable scene markers", "Allow scene markers to be crafted, placed, and edited. Markers already in the world keep loading either way. Requires restart because the recipe and the client overlay are startup-shaped.", ConfigAdminReloadBehavior.RestartRequired, c => c.EnableSceneMarkers, (c, v) => c.EnableSceneMarkers = v),
            ...
            Bool("Teleportation.TopRequireTemporalGear", "Home/Spawn", "Top requires temporal gear", "Require a temporal gear for /top teleports. Checked before warmup and consumed only when the teleport completes.", ConfigAdminReloadBehavior.Live, c => Teleportation(c).TopRequireTemporalGear, (c, v) => Teleportation(c).TopRequireTemporalGear = v),
